...
| z-5   | WebGL recursive monolith    | 5       | Fixed object + orbit/dolly camera + restrained material response |
...
- Scroll-driven meaning happens inside R3F through a damped visual scroll value, orbit/dolly camera sampling, and subtle scroll-velocity material response. The object remains fixed and stable; never apply scroll transforms to the fixed DOM wrapper.
...
| WebGL monolith           | Orbit/dolly camera + restrained pointer parallax | Fixed object, stage-aware camera position/look-at/roll, and low-amplitude material response |
...
- `sceneStateRef.scrollProgress` drives `sampleNarrativeCameraPose()`, which combines orbit angle, dolly distance, height, look-at drift, and roll across the five narrative stages.
...
The WebGL object does not consume raw scroll progress directly. `Hero.tsx` keeps raw progress for content presence, then derives a damped visual progress for `sceneStateRef`. Camera movement samples that continuous `scrollProgress` through `src/lib/cameraMotion.ts`, while `stageIndex` / `stageProgress` continue to drive material state and narrative posture. This keeps the scroll feeling like the camera is observing the artifact from changing distance and angle, not sliding between flat waypoints.
...
| WebGL rendering | Three.js / R3F / drei | Orbit/dolly camera path, recursive monolith geometry, shader material, background scene |
...
- **Camera:** `sampleNarrativeCameraPose()` from `src/lib/cameraMotion.ts`, `fov 48` — restrained product-page observation rather than object physics. The path combines orbit, dolly, target drift, and roll.
...
- `sampleNarrativeCameraPose()` defines stage-aware camera position, look-at target, and roll through a compact/desktop orbit-dolly profile. Runtime sampling remains continuous and avoids section-boundary snaps.
...
- `Hero.tsx` waits for fonts and the GLB `MonolithModel` mount signal before opening the scene gate. The `scene-preloader` then fades out while `.webgl-shell` eases from slight blur/dim/scale into the final camera view, preventing a hard cut into the page.
