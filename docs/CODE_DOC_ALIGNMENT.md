...
WebGL 当前保持单一 recursive fossil monolith 焦点。主体是固定 GLB 物体，滚动叙事由 camera rail、材质响应和少量 stage 姿态完成。页面色彩与文案当前是 dream valley interface 方向：暗谷底色、烟棕深度、深森林冷调、旧金微光、暖白文字。当前没有房间、重力、碰撞、刚体、Rapier runtime 或物理玩具。
...
| `README.md` | 单 WebGL scene + recursive monolith 焦点 | `WebGLSlab.tsx` 仅包含 `BgQuad` 与一个 GLB monolith group，无第二个可见 3D 主体 | 一致 |
...
| `DESIGN.md` | Georgia 字体、deep valley + warm off-white、不使用纯白 | 全局 token 与组件样式保持 Georgia 和 `#F0E7D7` | 一致 |
...
- `WebGLSlab` 只进行一次场景渲染：`BgQuad` 的固定背景 + recursive fossil monolith group。
...
- `Hero.tsx` 通过 fonts ready + `WebGLSlab` `onSceneReady` 卸载 `scene-preloader`，避免首屏黑帧或加载层残留。
