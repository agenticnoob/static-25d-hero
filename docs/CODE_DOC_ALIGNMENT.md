...
WebGL 当前保持单一 recursive fossil monolith 焦点。主体是固定 GLB 物体，滚动叙事由 orbit/dolly camera path、材质响应和少量 stage 姿态完成。页面 DOM/chrome 色彩当前是 `#142334` + `#baccd9` 双色撞色方向；WebGL canvas 透明，背景由网站 CSS 承担，monolith 只保留一盏主 `directionalLight`、recursive shader 高光、procedural stone texture 和 `ContactShadows`。当前没有房间、重力、碰撞、刚体、Rapier runtime 或物理玩具。
...
| `DESIGN.md`          | WebGL wrapper 固定，滚动变化发生在 R3F scene 内                              | `Hero.tsx` 渲染固定 WebGL wrapper，`sceneStateRef` 传入 `WebGLSlab`，scene 内通过 `sampleNarrativeCameraPose()` 和 shader uniforms 表达变化  | 一致             |
...
- `getCinematicScrollStage` 保留阶段起止 hold 区域给材质和叙事状态使用；camera path 直接采样 damped `scrollProgress`，避免阶段边界停顿。
...
- `Hero.tsx` 通过 fonts ready + GLB `MonolithModel` mount ready 打开 scene gate；`scene-preloader` 保留到淡出结束，`.webgl-shell` 同步从 slight blur/dim/scale 进入最终视图，避免模型未出现或硬切时页面已经可滚动。
...
- `npm run lint` 当前执行 ESLint；`npm run format:check` 执行 Prettier 风格检查；`npm run typecheck` 单独执行 TypeScript 静态检查。
