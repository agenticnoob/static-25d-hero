...
block("monolith_core", root, [0, 0, 0], [1.06, 2.34, 0.42], stone, 0.035, 3, 0.01);
block("broken_plinth", root, [-0.03, -1.24, 0.035], [1.28, 0.34, 0.5], stone, 0.045, 3, 0.014);
block("top_cap", root, [-0.05, 1.06, 0.035], [1.08, 0.34, 0.48], stone, 0.035, 2, 0.01);
block("left_mass", root, [-0.62, -0.34, 0.03], [0.22, 1.18, 0.48], stone, 0.028, 2, 0.009);
block("right_mass", root, [0.62, 0.1, 0.02], [0.24, 1.58, 0.46], stone, 0.03, 2, 0.009);

[
  [0.02, 0.15, 0.244, 0.74, 1.38, 0.036],
  [0.02, 0.02, 0.282, 0.54, 0.94, 0.038],
  [0.04, -0.1, 0.322, 0.36, 0.56, 0.044],
  [0.05, -0.18, 0.37, 0.2, 0.28, 0.052],
].forEach((spec, index) => {
  block(
    `nested_shadow_${index}`,
    root,
    spec.slice(0, 3),
    spec.slice(3),
    deepShadow,
...
[
  ...
  [0.4, 0.15, 0.318, 0.035, 1.48],
  [0.02, 0.88, 0.318, 0.76, 0.035],
  [0.02, -0.58, 0.318, 0.76, 0.035],
  [-0.25, 0.03, 0.358, 0.03, 0.94],
  [0.31, 0.03, 0.358, 0.03, 0.94],
  [0.03, 0.49, 0.358, 0.56, 0.03],
  [0.03, -0.43, 0.358, 0.56, 0.03],
  [-0.14, -0.1, 0.402, 0.026, 0.52],
  [0.23, -0.1, 0.402, 0.026, 0.52],
  [0.05, 0.15, 0.402, 0.36, 0.026],
  [0.05, -0.35, 0.402, 0.36, 0.026],
].forEach((spec, index) => {
  lineBlock(`nested_rim_${index}`, root, spec[0], spec[1], spec[2], spec[3], spec[4], 0.07, stone);
});

[
  [-0.5, -0.72, 0.35, 0.22, 0.13, 0.12],
  [-0.58, -1.02, 0.34, 0.18, 0.18, 0.14],
  [0.5, 0.48, 0.34, 0.18, 0.56, 0.12],
  [0.22, 0.42, 0.41, 0.25, 0.16, 0.12],
  [-0.02, -0.78, 0.4, 0.2, 0.13, 0.11],
].forEach((spec, index) => {
...
[
  ...
  [-0.04, 1.22, 0.518, 0.88, 0.01],
  [0.02, -1.06, 0.522, 0.98, 0.012],
  [-0.36, -0.58, 0.528, 0.72, 0.01],
  [0.02, 0.88, 0.528, 0.76, 0.01],
].forEach((spec, index) => {
  lineBlock(`worn_edge_${index}`, root, spec[0], spec[1], spec[2], spec[3], spec[4], 0.01);
});

[
  [0.61, 0.54, 0.526, 0.24, -0.35],
  [0.63, 0.2, 0.526, 0.18, 0.18],
